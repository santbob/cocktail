...
class Store {
  ...
  @action setCurrentDrinkIndex(index) {
    ...
    if (
      !this.searchTerm &&
      this.drinks.length &&
      this.drinks[this.currentDrinkIndex] &&
      this.drinks[this.currentDrinkIndex].idDrink
    ) {
  ...
  @action fetchAllDrinks() {
    this.loading = true;
    fetch(
      "http://www.thecocktaildb.com/api/json/v1/18234/filter.php?a=Alcoholic"
    )
      .then(res => res.json())
      .then(json => {
        if (json.drinks) {
  ...
  @action fetchDrinkInfo(drinkId) {
    if (!this.drinkDetails[drinkId]) {
      this.loading = true;
      fetch(
        `http://www.thecocktaildb.com/api/json/v1/18234/lookup.php?i=${drinkId}`
      )
        .then(res => res.json())
        .then(json => {
  ...
  @action searchByDrink(searchTerm) {
    this.loading = true;
    fetch(
      `http://www.thecocktaildb.com/api/json/v1/18234/search.php?s=${searchTerm}`
    )
      .then(res => res.json())
      .then(json => {
        return json.drinks;
